...
class SM_C_H(object):
    ...
    def __init__(self, params, nbins=100, q=1, f_wilt=0.05, f_star=0.95, constraints=True, stress_type='dynamic'):
        ...
        [self.T_GS, self.rf_alpha, self.rf_lambda, self.Eo, self.Td,
          self.LAI, self.Fpar, self.RAI, self.hc, self.Zm,
        ...
        #print(self.epsilon, self.epsilon_static, self.get_mean_t(self.p0), self.mean_dynamic_water_stress(self.p0))
    ...
    def mean_stress_duration(self, p0):
        P0 = np.cumsum(p0)
        l = (len(p0) - 1) 
        P_star = P0[np.int32(np.rint(self.s_star * l))]
        p_star = p0[np.int32(np.rint(self.s_star * l))]
    ...
    def mean_dynamic_water_stress(self, p0, k=0.8):
        l = (len(p0) - 1)
        P0 = np.cumsum(p0)
        P_star = P0[np.int32(np.rint(self.s_star * l))]
